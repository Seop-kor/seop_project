...
var World = {
    ...
    loadPoisFromJsonData: function loadPoisFromJsonDataFn(poiData) {
        ...
        var placesInRange = poiData.length;

        if(World.markerList.length > 0){
            var maxRangeMeters = 500;

                    /* Number of places within max-range. */
            placesInRange = World.getNumberOfVisiblePlacesInRange(maxRangeMeters);

                    /* Update culling distance, so only places within given range are rendered. */
            AR.context.scene.cullingDistance = 500;

                    /* Update radar's maxDistance so radius of radar is updated too. */
            PoiRadar.setMaxDistance(500);
        }

        /* Loop through POI-information and create an AR.GeoObject (=Marker) per POI. */
        for (var currentPlaceNr = 0; currentPlaceNr < placesInRange; currentPlaceNr++) {
            var singlePoi = {
    ...
    locationChanged: function locationChangedFn(lat, lon, alt, acc) {
        ...
        if (!World.initiallyLoadedData) {
            ...
            World.loadPoisFromJsonData //여기까지
        }
